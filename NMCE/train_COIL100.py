...
data_dict = torch.load('COIL100.pth')
...
def validate():
    ...
    preds = preds.cpu()
    args = nn.Identity()
    args.n_clusters = 94 #95: 87.98 94: 88.45
    args.gam = 200 #200 gives 84.5? 200, 0.5: 86.98?
    args.tau = 0.5
    args.algo = 'spams'
...
net = SubspaceClusterNetwork('resnet10imgs',z_dim,n_clusters,norm_p=2).cuda()
...
# criterion = MaximalCodingRateReduction(eps=0.1,gamma=1.0).cuda()
criterion = TotalCodingRate(eps=0.001)
# criterion = NucNorm(eps=0.)
criterion_z = Z_loss()
