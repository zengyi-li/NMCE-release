...
def kmeans(args, train_features, train_labels, test_features=None, test_labels=None):
    ...
    """
    clustermd = cluster.KMeans(n_clusters=args.n_clusters, random_state=10)
    clustermd.fit(train_features)
    plabels = clustermd.labels_
#     acc = clustering_accuracy(train_labels, plabels)
    _,_, acc = cluster_match(torch.from_numpy(plabels),train_labels,n_classes=args.n_clusters)
...
def ensc(args, train_features, train_labels, test_features=None, test_labels=None):
    ...
    """
    clustermd = ElasticNetSubspaceClustering(n_clusters=args.n_clusters, gamma=args.gam, tau=args.tau, active_support=True, algorithm=args.algo)
    clustermd.fit(train_features)
    plabels = clustermd.labels_
#     acc = clustering_accuracy(train_labels, plabels)
    _,_, acc = cluster_match(torch.from_numpy(plabels),train_labels,n_classes=args.n_clusters)
...
def cluster_match(cluster_mtx,label_mtx,n_classes=10,print_result=False):
...
def clustering_accuracy(labels_true, labels_pred):
    ...
    """
    labels_true, labels_pred = metrics.cluster.check_clusterings(labels_true, labels_pred)
    # value = supervised.contingency_matrix(labels_true, labels_pred, sparse=False)
    value = metrics.cluster.contingency_matrix(labels_true, labels_pred)
    [r, c] = optimize.linear_sum_assignment(-value)
